...
const App = () => {
  ...
  const calculateFixedYield = () => {
    ...
    const fixedYieldPercent = (exchangeRates.mgpWSTMGP / burnRate - 1) * 100;
  ...
  return <>
    ...
    <div className="flex h-screen bg-gray-900 text-white">
      ...
      <div className="grow overflow-auto">
        <Header ens={wallet.ens} chain={wallet.chain} page={page} setPage={setPage} account={wallet.account} isConnecting={wallet.isConnecting} setChain={updateWallet.setChain} connectWallet={updateWallet.connectWallet} mgpBalance={balances.user.MGP} rmgpBalance={balances.user.wstMGP} ymgpBalance={balances.user.yMGP} cmgpBalance={balances.user.cMGP} />
        <div className="flex flex-col gap-6 p-4 md:mx-16 md:p-6 lg:mx-24 xl:mx-28">
          {(() => {
            ...
            if (page === "vote") return <div className="flex flex-col h-screen bg-gray-900 text-white">
              <ConnectWallet connectRequired={wallet.connectionRequired} isConnecting={wallet.isConnecting} connectWallet={updateWallet.connectWallet} />
              <ErrorCard error={error} setError={setError} />
              <VotePage vmgpBalance={balances.user.vMGP} vmgpSupply={supplies.vMGP} lvmgpSupply={supplies.lvMGP} reefiMgpLocked={balances.wstMGP.MGP} onVote={actions.onVote} ymgpBalance={balances.user.yMGP} />
            </div>;
            ...
            return <>
              <TokenCards mgpLocked={supplies.vlMGP} mgpPrice={prices.MGP} mgpSupply={supplies.MGP} rmgpSupply={supplies.wstMGP} ymgpSupply={supplies.yMGP} ymgpLocked={supplies.lyMGP} reefiMGPLocked={balances.wstMGP.MGP} mgpRmgpCurveRate={exchangeRates.mgpWSTMGP} rmgpYmgpCurveRate={exchangeRates.wstmgpYMGP} ymgpVmgpCurveRate={exchangeRates.ymgpVMGP} vmgpSupply={supplies.vMGP} />
              <Features mgpAPR={rewards.vlmgpAPR} lockedYmgpAPY={lockedYmgpAPY} mgpPrice={prices.MGP} vmgpSupply={supplies.vMGP} reefiLockedMGP={balances.wstMGP.MGP} vmgpMGPCurveRate={0.5} />
              ...
              <Card>
                <div className="flex flex-col-reverse justify-between gap-2 lg:flex-row w-full">
                  <div className="flex rounded-lg bg-gray-700 p-1">
                    ...
                    <Button size="sm" variant={page === "deposit" ? "primary" : "clear"} onClick={() => setPage(page === "deposit" ? undefined : "deposit")} type="button" tooltip="Deposit your Magpie and earn auto compounded & liquid yield">Get wstMGP</Button>
                    {balances.user.wstMGP > 0n && <Button size="sm" variant={page === "redeem" ? "primary" : "clear"} onClick={() => setPage(page === "redeem" ? undefined : "redeem")} type="button" tooltip="Withdraw the underlying MGP from wstMGP">Withdraw MGP</Button>}
                    <Button size="sm" variant={page === "fixedYield" ? "primary" : "clear"} onClick={() => setPage(page === "fixedYield" ? undefined : "fixedYield")} type="button" tooltip="Earn fixed interest by buying depegged wstMGP and redeeming">Fixed Yield</Button>
                  </div>
                  <div className="flex h-min flex-row-reverse">
                    <div className="flex gap-1">
                      ...
                      <YieldBadge value={calculateFixedYield()} asset="wstMGP Fixed Yield" breakdown={[{ value: `${((exchangeRates.mgpWSTMGP / (Number(balances.wstMGP.MGP) / Number(supplies.wstMGP)) - 1) * 100).toFixed(2)}%`, asset: "wstMGP Discount", logo: coins.wstMGP.icon }]} logo={coins.wstMGP.icon} />
                    </div>
                  </div>
                </div>
                ...
                {page === "fixedYield" && <FixedYieldPage mgpAPR={rewards.vlmgpAPR} balances={balances} setSend={amountsActions.setSend} send={amounts.send} allowances={allowances} chain={wallet.chain} rmgpSupply={supplies.wstMGP} unlockSchedule={withdraws.reefi.unlockSchedule} approve={actions.approve} mintWETH={actions.mintWETH} swap={actions.swap} curveAmounts={amounts.curve} supplies={supplies} curveBuy={actions.curveBuy} nativeSwap={actions.nativeSwap} />}
                ...
                <div className="flex flex-col-reverse justify-between gap-2 lg:flex-row mt-8 w-full">
                  <div className="flex rounded-lg bg-gray-700 p-1">
                    <Button size="sm" variant={page === "convert" ? "primary" : "clear"} onClick={() => setPage(page === "convert" ? undefined : "convert")} type="button" tooltip="Convert your wstMGP for yMGP for better yield potential">Get yMGP</Button>
                    ...
                    {balances.user.yMGP > 0n && <Button size="sm" variant={page === "redeemYMGP" ? "primary" : "clear"} onClick={() => setPage(page === "redeemYMGP" ? undefined : "redeemYMGP")} type="button" tooltip="Convert your yMGP back to wstMGP">Redeem yMGP</Button>}
                  </div>
                  <div className="flex h-min flex-row-reverse">
                    <div className="flex gap-1">
                      ...
                      <YieldBadge apy={Number(balances.wstMGP.MGP) * aprToApy(rewards.vlmgpAPR) * 0.05 / Number(supplies.lyMGP) + aprToApy(rewards.vlmgpAPR) * 0.9} asset="Locked yMGP" breakdown={[{ apy: aprToApy(rewards.vlmgpAPR) * 0.9, asset: "wstMGP", logo: coins.wstMGP.icon }, { apr: Number(balances.wstMGP.MGP) * rewards.vlmgpAPR * 0.05 / Number(supplies.lyMGP), asset: "Boosted vlMGP", logo: vlMGP }, { asset: "yMGP Withdraws", value: "Variable", logo: coins.yMGP.icon }]} suffix='+' logo={coins.yMGP.icon} />
                    </div>
                  </div>
                </div>
                ...
                {page === "lock" && <LockPage ymgpBalance={balances.user.yMGP} setSend={amountsActions.setSend} send={amounts.send} lockYMGP={actions.lockYMGP} mgpAPR={rewards.vlmgpAPR} reefiLockedMGP={balances.wstMGP.MGP} ymgpLocked={supplies.lyMGP} />}
                ...
                <div className="flex flex-col-reverse justify-between gap-2 lg:flex-row mt-8 w-full">
                  ...
                  <div className="flex h-min flex-row-reverse">
                    <div className="flex gap-1">
                      <Badge title="Vote Multiplier" value={`${formatNumber(Number(balances.wstMGP.MGP) / Number(supplies.vMGP), 2)}x`} breakdown={[{ asset: "Reefi's vlMGP", logo: vlMGP, value: formatNumber(formatEther(balances.wstMGP.MGP)) }, { asset: "Votable vMGP", value: formatNumber(formatEther(supplies.vMGP)), logo: coins.vMGP.icon }]} />
                    </div>
                  </div>
                </div>
                    ...
                    <div className="flex gap-1">
                      <YieldBadge
                        ...
                        breakdown={[
                          ...
                          { apy: aprToApy(rewards.vlmgpAPR) * 0.9, asset: `${(100 * Number(balances.curve.wstMGP) * Number(balances.wstMGP.MGP) / Number(supplies.wstMGP) / (Number(balances.curve.MGP) + Number(balances.curve.wstMGP) * Number(balances.wstMGP.MGP) / Number(supplies.wstMGP) + Number(balances.curve.yMGP) * Number(balances.wstMGP.MGP) / Number(supplies.wstMGP))).toFixed(0)}% wstMGP`, logo: coins.wstMGP.icon },
                          { apy: aprToApy(rewards.vlmgpAPR) * 0.9, asset: `${(100 * Number(balances.curve.yMGP) * Number(balances.wstMGP.MGP) / Number(supplies.wstMGP) / (Number(balances.curve.MGP) + Number(balances.curve.wstMGP) * Number(balances.wstMGP.MGP) / Number(supplies.wstMGP) + Number(balances.curve.yMGP) * Number(balances.wstMGP.MGP) / Number(supplies.wstMGP))).toFixed(0)}% yMGP`, logo: coins.yMGP.icon },
                ...
                {page === "supplyLiquidity" && <SupplyLiquidityPage mgpBalance={balances.user.MGP} rmgpBalance={balances.user.wstMGP} ymgpBalance={balances.user.yMGP} mgpCurveBalance={balances.curve.MGP} rmgpCurveBalance={balances.curve.wstMGP} ymgpCurveBalance={balances.curve.yMGP} setLPAmounts={amountsActions.setLP} mgpLPAmount={amounts.lp.MGP} rmgpLPAmount={amounts.lp.wstMGP} ymgpLPAmount={amounts.lp.yMGP} supplyLiquidity={actions.supplyLiquidity} />}
              </Card>
              <QASection />
              <div className="flex gap-6">
                <PegCard token="wstMGP" targetToken="MGP" spread={100 / exchangeRates.mgpWSTMGP / exchangeRates.wstmgpMGP - 100} softPeg={true}
                  rates={[
                    { label: "Mint", value: Number(balances.wstMGP.MGP) / Number(supplies.wstMGP), color: "green", required: true },
                    { label: "Buy", value: 1 / exchangeRates.mgpWSTMGP, color: "blue", required: true },
                    { label: "Sell", value: exchangeRates.wstmgpMGP, color: "red", required: true },
                    { label: "Orig Mint", value: 1, color: "emerald" },
                    { label: "Target", value: Number(supplies.wstMGP) / Number(balances.wstMGP.MGP) / (1 + aprToApy(rewards.vlmgpAPR) * (withdraws.reefi.unlockSchedule.length === 6 ? Number(withdraws.reefi.unlockSchedule[0]?.endTime) - Date.now() / 1000 + 60 * 60 * 24 * 30 * 2 : 60 * 60 * 24 * 30 * 2 / (60 * 60 * 24)) / 365), color: "purple", required: true }
                  ]}
                />
                <PegCard token="yMGP" targetToken="wstMGP" spread={100 / exchangeRates.wstmgpYMGP / exchangeRates.ymgpWSTMGP - 100}
                  rates={[
                    { label: "Mint", value: 1, color: "green", required: true },
                    { label: "Buy", value: 1 / exchangeRates.wstmgpYMGP, color: "blue", required: true },
                    { label: "Sell", value: exchangeRates.ymgpWSTMGP, color: "red", required: true },
