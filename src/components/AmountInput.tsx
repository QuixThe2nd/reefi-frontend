...
import { formatEther, formatNumber, parseEther } from "../utilities";
...
export const AmountInput = ({ label, balance, value, onChange, token, placeholder = "0" }: AmountInputProperties) => <div className="mb-4">
  ...
  <div className="flex items-center justify-between rounded-lg bg-gray-900 p-4">
    <input className="w-3/4 bg-transparent text-xl outline-none" max={formatEther(balance)} onChange={event => onChange(parseEther(Number.isNaN(Number.parseFloat(event.target.value)) ? 0 : Number.parseFloat(event.target.value)))} placeholder={placeholder} type="text" value={value === 0n ? undefined : formatEther(value)} />
