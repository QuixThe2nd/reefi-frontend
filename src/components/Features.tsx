...
export const Features = memo(({ mgpAPR, lockedYmgpAPY, mgpPrice, vmgpSupply, reefiLockedMGP, vmgpMGPCurveRate }: Properties): ReactElement => <div className="flex flex-col items-center">
  <div className="mb-4 grid w-full grid-cols-2 gap-4 md:grid-cols-4">
    <div className="rounded-lg border border-green-600/30 bg-gradient-to-r from-green-600/20 to-blue-600/20 p-3 text-center">
      <div className="text-lg font-bold text-green-400">{(100 * mgpAPR).toFixed(2)}%</div>
      <div className="text-xs text-gray-400">Base Magpie Yield</div>
    </div>
    <div className="rounded-lg border border-green-600/30 bg-gradient-to-r from-green-600/20 to-blue-600/20 p-3 text-center">
      <div className="text-lg font-bold text-green-400">{(100 * aprToApy(mgpAPR)).toFixed(2)}%+</div>
    ...
    <div className="rounded-lg border border-green-600/30 bg-gradient-to-r from-green-600/20 to-blue-600/20 p-3 text-center">
      <div className="text-lg font-bold text-green-400">&lt;${(1000 * mgpPrice * vmgpMGPCurveRate * Number(vmgpSupply) / Number(reefiLockedMGP)).toFixed(2)} vs ${(1000 * mgpPrice).toFixed(2)}</div>
