...
export const Header = memo((): ReactElement => {
  ...
  return <div className="sticky top-0 z-10 flex w-full items-center justify-between bg-gray-800 p-4">
    ...
    {wallet.account === undefined ? <button className="rounded-lg bg-green-600 px-4 py-2 transition-colors hover:bg-green-700" disabled={wallet.isConnecting} onClick={wallet.connectWallet} type="button">{wallet.isConnecting ? "Connecting..." : "Connect Wallet"}</button> : <div className="flex items-center space-x-4">
      <TokenBalances />
      <div className="rounded-lg bg-green-600/20 px-3 py-2 text-sm text-green-400">{wallet.ens ?? `${wallet.account.slice(0, 6)}...${wallet.account.slice(-4)}`}</div>
      <select
