...
export const Navigation = ({ balances, page, setPage, rewards, exchangeRates, supplies, bonds }: Properties) => {
  const [activeSection, setActiveSection] = useState<"core" | "advanced" | "liquidity" | undefined>();
  ...
  return activeSection ? <div className="space-y-6">
    <div className="flex items-center gap-4">
      <button className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors group" onClick={() => setActiveSection(undefined)} type="button">
      ...
      <h2 className="text-xl font-semibold text-white">
        {activeSection === "core" && "MGP Staking"}
        {activeSection === "advanced" && "MGP Vaults"}
        {activeSection === "liquidity" && "Liquidity & Governance"}
      </h2>
    </div>
    <div className="flex justify-between">
      ...
      <div className="flex gap-2 pt-2">
        ...
        {activeSection === "liquidity" && <YieldBadge apy={cmgpAPY} asset="cMGP" breakdown={cmgpBreakdown} logo={coins.cMGP.icon} />}
      </div>
    </div>
  </div> : <div className="space-y-4">
    <h2 className="text-2xl font-bold text-white mb-4">What would you like to do?</h2>
    <div className="space-y-3">
      <button className="w-full group flex items-center justify-between p-6 bg-gray-800/50 hover:bg-gray-800 rounded-lg border border-gray-700/50 hover:border-gray-600 transition-all duration-200" onClick={() => setActiveSection("core")} type="button">
      ...
      <button className="w-full group flex items-center justify-between p-6 bg-gray-800/50 hover:bg-gray-800 rounded-lg border border-gray-700/50 hover:border-gray-600 transition-all duration-200" onClick={() => setActiveSection("liquidity")} type="button">
        <div className="flex-1 text-left">
          <div className="flex items-center gap-3 mb-2">
