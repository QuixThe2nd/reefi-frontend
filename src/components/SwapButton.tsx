...
export const SwapButton = ({ buy, nativeSwap, label, tokenIn, tokenOut }: { buy: () => void, nativeSwap: undefined | (() => void), label: string | undefined, tokenIn: Coins | 'ETH', tokenOut: Coins }): ReactElement => {
  ...
  if ((tokenIn === 'MGP' && tokenOut ==='rMGP') || (tokenIn === 'rMGP' && tokenOut ==='yMGP') || (tokenIn === 'rMGP' && tokenOut ==='MGP')) {
    ...
    return <div className="grid grid-cols-2 gap-2">
      <div>
        ...
        <button type="submit" className="py-2 rounded-lg transition-colors bg-green-600 hover:bg-green-700 h-min w-full text-xs md:text-md" onClick={nativeSwap}>{label} ({formatEther(BigInt(Number(amounts.send)*nativeRate)).toFixed(4)} {tokenOut})</button>
      </div>
  ...
  if (tokenIn === 'ETH') return <button type="submit" className="py-2 rounded-lg transition-colors bg-green-600 hover:bg-green-700 h-min w-full text-xs md:text-md" onClick={() => actions.mintWETH()}>Mint WETH</button>
  return <>
    ...
    <button type="submit" className="py-2 rounded-lg transition-colors bg-green-600 hover:bg-green-700 h-min w-full text-xs md:text-md" onClick={() => actions.swap(contracts[wallet.chain][tokenIn].address, contracts[wallet.chain].MGP.address)}>Swap to MGP With Odos</button>
  </>
}