...
export const TokenCard = ({ symbol, description, price, supply, underlying, underlyingSymbol, voteMultiplier, locked, marketRate }: Properties): ReactElement => <Card padding={4}>
  <div className="w-full">
    ...
    <div className="col-span-2 mt-2 grid grid-cols-1 gap-1 sm:grid-cols-2 xl:mt-0">
      <TokenStat detail={`${formatNumber(formatEther(supply, decimals[symbol]), 4)} ${symbol}`} title="Supply" />
      {price !== undefined && <TokenStat detail={`$${price.toFixed(4)}`} title="Price" />}
      {underlying !== undefined && underlyingSymbol !== undefined && underlying !== supply && <TokenStat detail={`${formatNumber(formatEther(underlying), 4)} ${underlyingSymbol}`} title="TVL" />}
