...
export const depositMGP = ({ allowances, send, setError, chain }: Properties) => (writeContract: UseWriteContractReturnType<typeof wagmiConfig>["writeContract"]) => {
  if (allowances.wstMGP_MGP < send) return setError("Allowance too low");
  writeContract({ abi: ABIs.wstMGP, address: contracts[chain].wstMGP, functionName: "deposit", args: [send] });
};
