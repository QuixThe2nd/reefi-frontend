...
export const depositRMGP = ({ allowances, send, setError, chain }: Properties) => (writeContract: UseWriteContractReturnType<typeof wagmiConfig>["writeContract"]) => {
  if (allowances.wstMGP_MGP < send) return setError("Allowance too low");
  writeContract({ abi: ABIs.yMGP, address: contracts[chain].yMGP, functionName: "deposit", args: [send] });
};
