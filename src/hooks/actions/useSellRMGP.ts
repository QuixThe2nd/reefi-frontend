...
export const useSellRMGP = <Clients extends Record<Chains, WalletClient & PublicActions> | undefined>({ account, allowances, balances, chain, clients, sendAmount, setConnectRequired, setError, writeContracts }: Props<Clients>): () => void => {
  ...
  const sellYMGP = useCallback(async (): Promise<void> => {
    ...
    if (allowancesRef.current.curve.yMGP[0] < sendAmountRef.current) return setErrorRef.current('Allowance too low')
