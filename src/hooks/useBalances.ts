...
export const useBalances = ({ wallet }: Readonly<{ wallet: UseWallet }>): UseBalances => {
  ...
  const updateBalances: UpdateBalances = {
    ...
    WETH: () => wallet.account === undefined ? Promise.resolve() : contracts[wallet.chain].WETH.read.balanceOf([wallet.account]).then(WETH => {
      setBalances(() => ({ WETH }));
    }),
    cMGP: () => wallet.account === undefined ? Promise.resolve() : contracts[wallet.chain].cMGP.read.balanceOf([wallet.account]).then(cMGP => {
      setBalances(() => ({ cMGP }));
    }),
    ...
    rMGP: () => wallet.account === undefined ? Promise.resolve() : contracts[wallet.chain].rMGP.read.balanceOf([wallet.account]).then(rMGP => {
      setBalances(() => ({ rMGP }));
    }),
