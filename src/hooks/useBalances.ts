...
export const useBalances = ({ account, chain }: { readonly account: `0x${string}` | undefined, readonly chain: Chains }): Balances => {
  const [mgp, updateMGP] = useUpdateable(() => account === undefined ? 0n : contracts[chain].MGP.read.balanceOf([account]), [contracts, account], 0n)
  const [rmgp, updateRMGP] = useUpdateable(() => account === undefined ? 0n : contracts[chain].RMGP.read.balanceOf([account]), [contracts, account], 0n)
  const [ymgp, updateYMGP] = useUpdateable(() => account === undefined ? 0n : contracts[chain].YMGP.read.balanceOf([account]), [contracts, account], 0n)
  // const [vmgp, updateVMGP] = useUpdateable(() => account === undefined ? 0n : contracts[chain].VMGP.read.balanceOf([account]), [contracts, account], 0n)
  const [cmgp, updateCMGP] = useUpdateable(() => account === undefined ? 0n : contracts[chain].CMGP.read.balanceOf([account]), [contracts, account], 0n)
