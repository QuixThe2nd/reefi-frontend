...
export function useExchangeRates({ locked, wallet, supplies }: Props): UseExchangeRates {
  ...
  const [mgpYMGP] = useUpdateable(async () => { return Number(await contracts[wallet.chain].CMGP.read.get_dy([0n, 2n, parseEther(0.000_01)], { account: wallet.account }))/Number(parseEther(0.000_01)) }, [contracts, wallet.chain], 'mgpYMGP curve', 0)
  ...
  const [ymgpMGP] = useUpdateable(async () => { return Number(await contracts[wallet.chain].CMGP.read.get_dy([2n, 0n, parseEther(0.000_01)], { account: wallet.account }))/Number(parseEther(0.000_01)) }, [contracts, wallet.chain], 'ymgpMGP curve', 0)
