...
export const useLocked = ({ chain, account }: { readonly chain: Chains, readonly account: `0x${string}` | undefined }): Locked => {
  ...
  const [mgpARB, updateMGPARB] = useUpdateable(() => contracts[42161].VLMGP.read.totalLocked(), [contracts, account], 0n)
  ...
  const [userYMGP, updateUserYMGP] = useUpdateable(() => account !== undefined ? contracts[chain].YMGP.read.lockedBalances([account]) : 0n, [contracts, account], 0n)
